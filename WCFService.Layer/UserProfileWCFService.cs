...
using MasterBalc = BussinessAccessLayer.UserProfileBAL;
using MasterEntity = EntityLayer.UserProfileEntity;
using MasterInterface = WCFContractLayer.UserProfileWCFContract;
...
{
    [ServiceBehavior(Namespace = "UserProfile_WCFContract", Name = "UserProfile_WCFService", InstanceContextMode = InstanceContextMode.PerCall)]
	[AspNetCompatibilityRequirements(RequirementsMode = AspNetCompatibilityRequirementsMode.Allowed)]
	public class UserProfileWCFService : MasterInterface
	{
		...
		public OperationResult Insert(MasterEntity oEntity)
		{
			...
			try
			{
				...
				obj.Data.Add((obj.Result) ? ht[MasterEntity.Fields.USERID] : -1);
			}
			catch (Exception ex)
			{
				obj.Message = ex.Message;
